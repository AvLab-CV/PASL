...
def main_face():
    ...
    while cap.isOpened():
        ...
        if preds is not None:
            ...
            frame_cropped = crop_face_with_landmarks(frame, landmarks)
            ...
            frame_resized = cv2.resize(frame_cropped, (512, 512))  # 調整為適當的大小以匹配右側結果的高度
            ...
            # frame_resized = cv2.resize(frame_cropped, (1024, 1024))
            # combined_result = np.hstack((frame_resized, right_result))

            cv2.imshow('Webcam Demo', combined_result)
