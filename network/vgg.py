...
class VGG_Activations_2(nn.Module):
    """
    This class allows us to execute only a part of a given VGG network and obtain the activations for the specified
    feature blocks. Note that we are taking the result of the activation function after each one of the given indeces,
    and that we consider 1 to be the first index.
    """
    def __init__(self, vgg_network, feature_idx):
        super(VGG_Activations_2, self).__init__()
        features_2 = list(vgg_network.features)
        features_2.append(vgg_network.avgpool)
        features_3 = list(vgg_network.classifier)


        features = list(vgg_network.features)
        self.features = nn.ModuleList(features).eval()
        self.features_2 = nn.ModuleList(features_2).eval()
        self.features_3 = nn.ModuleList(features_3).eval()
        self.idx_list = feature_idx
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 512 * 7 * 7)
        for ii, model in enumerate(self.features_3):
            x = model(x)
            if ii == 3:
...
def vgg_face_state_dict():
    VGG_FACE = r'./network/vgg_face_dag.pth'
    default = torch.load(VGG_FACE)
